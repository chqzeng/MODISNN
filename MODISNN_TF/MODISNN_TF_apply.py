...
def GenHeader(strTemplate,strBand,strDir): #Generate a new header file for a given band
	#with fileinput.FileInput(strTemplate,inplace=True,backup='.bak') as file:
	with open(strTemplate,'r') as file:
		filedata=file.read()
	filedata=filedata.replace('_BAND_',strBand)
	with open(strDir+strTemplate.replace('_template',strBand),'w') as file:
		file.write(filedata)
...
def DN2ref(DN_array):  #input a numpy array
    """convert a input DN array from img files into the actuall reflectance, based on Rrs=0.05+2*10-6*DN"""
    invalid_mask = (DN_array >= 32767)| (DN_array <= -32767)
    Ref_array= 0.05+2e-6*DN_array
    Ref_array[invalid_mask]=-100; #np.nan
    return np.float32(Ref_array)
	
def Ref2DN(Ref_array):  #input a numpy array
    """convert a input Ref actuall reflectance to an array that write to img files , based on Rrs=0.05+2*10-6*DN"""
    DN_array=(Ref_array-0.05)/2e-6
    invalid_mask = (Ref_array >= 0.5)| (Ref_array <= -5e-3)
    DN_array[invalid_mask]=-32767; #np.nan
    return np.int16(DN_array)
    
def bitorderinvert(x):
    import struct
    return struct.unpack('>f', struct.pack('=f', x))
    
def MODISNN_MODISrhos_2_OLCIRef(strDir,lakeID='general'):
    ...
    path_img=strDir+'rhos_'+str(bandlist[0])+'.img'
    ...
    for idx in range(len(bandlist)):
        path_img=strDir+'rhos_'+str(bandlist[idx])+'.img'
    ...
    #Rrs_pred=Ref2DN(Rrs_pred)  #convert reflectance to DN that consistent with the input bands
    ...
    for idx in range(len(out_band_list)):  #write the NN predicted MERIS bands to replace some uncertainty layers
        ...
        path_img=strDir+'NN'+strband+'.img'
        ...
        GenHeader('NN_template.hdr',strband,strDir)  #str(MERSI_bands[idx])

MODISNN_MODISrhos_2_OLCIRef(r'./TestData/A2011253190500_NN.data/',lakeID='LW')